...
if(isset($_POST['email']) && $_POST['email'] !== '' and
    isset($_POST['first_name']) && $_POST['first_name'] !== '' and
       ...
       isset($_POST['confirm']) && $_POST['confirm'] !== '')
        {
          ...
          if($_POST['password'] === $_POST['confirm'])
          {
            ...
            if(strlen($_POST['password']) >= 5 )
            {
                ...
                if($user === false)
                {
                   ...
                   $query = "INSERT INTO project.users SET email = ?, first_name = ?, last_name = ?, password = ?, created_at = NOW();";
                   $statements = $pdo->prepare($query);
                   $password = password_hash($_POST['password'], PASSWORD_DEFAULT);
                   $statements->execute([$_POST['email'], $_POST['first_name'], $_POST['last_name'], $password]);
              ...
              $error = "!رمز عبور باید بزرگ تر از 5 باشد";

            }

          }else{

          $error = "!رمز عبور با تکرار یکی نیست";
          ...
          if(!empty($_POST))
          {
            $error = "!همه فیلد ها الزامی هستند";
          }
        }
